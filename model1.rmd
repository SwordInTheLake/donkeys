...
$$Weight = \beta_0e^{\beta_1I_{[BCS=1.5]}}e^{\beta_2I_{[BCS=2]}}e^{\beta_3I_{[BCS=2.5]}}e^{\beta_4I_{[BCS=3]}}e^{\beta_5I_{[BCS=3.5]}}e^{\beta_6I_{[BCS=4]}}e^{\beta_7I_{[Age<2]}}e^{\beta_8I_{[Age \in [2,5]]}}e^{\beta_9I_{[Age>5]}}Length^{\beta_{10}}Girth^{\beta_{11}}Height^{\beta_{12}}$$

## Interpretation 

* Toy example, donkey $i$ with $BCS = 3, \ Age = 3, \ Weight_i = \beta_0e^{\beta_4}e^{\beta_8}Length^{\beta_10}Girth^{\beta_{11}}Height^{\beta_{12}}$
* In other words, the BCS and Age category of the donkey give proportionate adjustments. These adjustments are determined by which categories the donkey falls into. 
...
* $Height^{\beta_{12}}$ is a proxy for frame of body, and therefore, the proportion of volume that consists of bones. This gives a proportionate adjustment to the weight.
...
* ToDo: Justify the lack of sex, perhaps using backward selection on this new model. This backward selection could also provide justification for keeping Height in the model, unlike the original model.

# Run model

* ToDo: Find out if lm implements above model for factors BCS and Age. I believe it does. 

```{r}
#m5 = lm(log(Weight) ~ BCS + Age + log(0.778*Length*(Girth^2) + 2.88*Legs), data=train)
m5 = lm(log(Weight) ~ BCS + Age + log(Length) + log(Girth) + log(Height), data=train)
#m5 = lm(Weight ~ Length*Girth^2, data=train)
summary(m5)
```

* We note that we get $\beta_{10} = 0.611, \beta_{11} = 1.47$, which are not equal to our idealized model, but close. This could indicate a cylinder with a "fatter" center, and a more ellipsoidal body. 
...
```{r}
predictions = predict.lm(m5, test, interval="confidence")
